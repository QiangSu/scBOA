...
├── test_assets/
│   └── Healthy_COVID19_PBMC.pkl # Model for automated testing
...
The repository's full history contains large model files from previous versions, which can make a standard clone slow. For the fastest setup, it is highly recommended to perform a **shallow clone**, which downloads only the latest version of the code:
...
# --- Analysis & Clustering Parameters ---
  ...
  --hvg_max_mean 3.0 \
  --hvg_min_disp 0.3 \
  --cas_aggregation_method consensus
```
