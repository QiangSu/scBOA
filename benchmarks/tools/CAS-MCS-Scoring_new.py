...
marker database CSV, and supports manual overrides via command line arguments.
...
def main(args):
    ...
    if args.marker_db_csv:
        try:
            ...
            # --- Modified Auto-detection & Override logic ---
            type_col = args.marker_db_celltype_col
            gene_col = args.marker_db_gene_col
            
            header = pd.read_csv(args.marker_db_csv, nrows=0).columns.tolist()

            # If user didn't provide overrides, try to auto-detect
            ...
            if not type_col or not gene_col:
                raise ValueError(f"Could not auto-detect marker DB columns from header: {header}. "
                                 "Please use --marker_db_celltype_col and --marker_db_gene_col manually.")
            
            print(f"       -> Validated format: TYPE='{type_col}', GENE='{gene_col}'")
            ...
            for cluster_id in adata.obs['leiden'].cat.categories:
                cluster_degs_for_ratio = {str(g).upper() for g in marker_names[cluster_id][:50]}
...
if __name__ == '__main__':
    ...
    # <<< START OF MODIFICATION: NEW ARGUMENTS >>>
    io_group.add_argument('--marker_db_celltype_col', type=str, default=None, help='(Optional) Manual override for the cell type column name in marker CSV.')
    io_group.add_argument('--marker_db_gene_col', type=str, default=None, help='(Optional) Manual override for the gene column name in marker CSV.')
    # <<< END OF MODIFICATION >>>
